//! This module contains pizauth's core central state.
//! [`AuthenticatorState`] is the global state, but mostly what one is interested in are
//! [`Account`]s and [`TokenState`]s.
//! These are (literally) locked together: every [`Account`] has a [`TokenState`] and vice versa.
//! However, a challenge is that we allow users to reload their config at any point:
//! we have to be very careful about associating an [Account] with a [`TokenState`],
//! as we don't want to hand out credentials for an old version of an account.
//!
//! To that end, we provide an abstraction [`AccountId`] which is a sort-of
//! "the current version of an [`Account`]".
//! Any change to the user's configuration of an [Account]
//! *or* a change to an [`Account`]'s associated [`TokenState`]
//! will cause the [`AccountId`] to change.
//! Every time a [`CTGuard`] is dropped/reacquired,
//! or [`tokenstate_replace`](CTGuard::tokenstate_replace) is called,
//! [`AccountId`]s must be revalidated.
//! Failing to do so will cause panics.
...
pub struct AuthenticatorState {
    ...
    /// The "global lock" protecting the config and current [`TokenState`]s.
    /// Can only be accessed via [`AuthenticatorState::ct_lock`].
    ...
    /// If an HTTPS server is running,
    /// its raw public key formatted in hex
    /// with each byte separated by `:`.
...
///      i.e. they will not be able to access [`Account`]s or [`TokenState`]s
...
impl<'a> CTGuard<'a> {
    ...
    /// Return a reference to the [`TokenState`] of `act_id`.
    /// The user must have validated `act_id` under the current [`CTGuard`].
    ///
    /// # Panics
    ///
    /// If `act_id` is not valid.
    ...
    /// Update the tokenstate for `act_id` to `new_tokenstate`
    /// returning a new [`AccountId`] valid for the new tokenstate,
    /// updating the tokenstate version.
    ///
    /// # Panics
    ///
    /// If `act_id` is not valid.
...
#[derive(Deserialize, Serialize, SchemaRead, SchemaWrite)]
/// The format of a dumped [`TokenState`].
/// Note that [`std::time::Instant`] instances are translated to [`std::time::SystemTime`] instances:
/// there is no guarantee that we can precisely represent the latter as the former,
/// so when conversions fail we default to setting values to [`std::time::Instant::now()`]
/// or [`std::time::SystemTime::now()`],
/// as appropriate, as a safe fallback.
